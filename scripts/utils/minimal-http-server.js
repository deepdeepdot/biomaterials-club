  ...
  const f = (err, stats) => {
    err
      ...
      ? (() => {
          ...
          const fileStream2 = fs.createReadStream(indexfilepath).pipe(res);
...
const server = (directory) => (port) => (serverUp) =>
  http.createServer(request(directory)).listen(port, serverUp);
